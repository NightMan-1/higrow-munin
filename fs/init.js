...
let cloudconnect = Timer.set(1000, Timer.REPEAT, function() {
  ...
  if (t < 100 && h < 100){
    print("DeviceId:",deviceId,"Temperature:",t,'*C',"Humidity:",h,'%',"Water:",ADC.read(SOILpin),"Light",ADC.read(LIGHTpin));

    if(deviceId!=="" && connected){
      GPIO.write(LedPin, 1);
      HTTP.query({
        url: 'http://192.168.1.2:8083/write', // replace with your own endpoint
